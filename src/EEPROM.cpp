...
/**
  * @brief  Removes variable data in EEPROM.
  *         Creates/merges with variable at address 0,
   *        creates dummy variable at adress 0 if itdoes not exists
  * @param  VirtAddress: Variable virtual address
  * @retval Success or error status:
  *			- EEPROM_SAME_VALUE: If new Data matches existing EEPROM Data
  *			- FLASH_COMPLETE: on success
  ...
  *			- Flash error code: on write Flash error
  */
uint16_t EEPROMClass::remove(uint16_t Address) {
  uint16_t backup0;
  uint16_t status;
  uint32_t pageBase, pageEnd;


  bool is_0exists = false;
  status = EEPROM_BAD_ADDRESS;
  //cannot remove variable at address 0 or 0xFFFF
  if ((Address == 0) || (Address == 0xFFFF)) return EEPROM_BAD_ADDRESS;
  if (Status == EEPROM_NOT_INIT)
    if (init() != EEPROM_OK)
      return Status;
  ...
  //check if variable with address 0 exists, make a backup   
  is_0exists = read(0, &backup0) == EEPROM_OK ? true : false;
  ...
  for (pageBase += 6; pageEnd >= pageBase; pageEnd -= 4)
  {
    if ((*(__IO uint16_t*)pageEnd) == Address)		// Compare the read address with the virtual address and zero all occurance
    {
      //set address to 0
      if ((status = FLASH_ProgramHalfWord(pageEnd, 0u)) != FLASH_COMPLETE) return status;
      //variable @Address found 
      status = EEPROM_OK;
    }
  }
  if ((status == EEPROM_OK) && is_0exists)
  {
    status = write(0, backup0);
    return status;
  }
  // Return ReadStatus value: (0: variable exist, 1: variable doesn't exist)
  return EEPROM_BAD_ADDRESS;
